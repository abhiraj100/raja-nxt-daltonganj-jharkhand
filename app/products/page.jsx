...
import { SlidersHorizontal, X, Search, ShoppingBag, Minus, Plus, Trash2, ChevronDown, MessageCircle, CheckCircle } from "lucide-react";
...
const WA_NUMBER = "918877085761"; // no +

// ─── Build WhatsApp message ────────────────────────────────────────────────
function buildWAMessage(cart, cartTotal) {
  const lines = [
    "🛍️ *New Order — Raja Nxt*",
    "━━━━━━━━━━━━━━━━━━━",
    ...cart.map((item, i) =>
...
function openWhatsApp(cart, cartTotal) {
  if (!cart.length) return;
  const msg = buildWAMessage(cart, cartTotal);
  window.open(`https://wa.me/${WA_NUMBER}?text=${msg}`, "_blank");
}

// ─── "Added to bag" popup ──────────────────────────────────────────────────
function AddedPopup({ item, cartTotal, cartCount, onCheckout, onClose }) {
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] w-[calc(100%-2rem)] max-w-sm animate-slide-up">
      <div className="bg-white rounded-2xl shadow-2xl border border-stone-100 overflow-hidden">
        ...
        <div className="h-0.5 bg-rose-100">
          <div className="h-full bg-rose-500 animate-shrink-width" style={{ animationDuration:"5s" }}/>
        </div>
        <div className="p-4 flex gap-3 items-center">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-charcoal line-clamp-1">"{item.name}" added! 🎉</p>
            <p className="text-xs text-stone-400 mt-0.5">{cartCount} item{cartCount>1?"s":""} · ₹{cartTotal.toLocaleString("en-IN")} total</p>
          </div>
        ...
        <div className="px-4 pb-4 flex gap-2">
          <button onClick={onCheckout}
            className="flex-1 flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors shadow-sm">
            <MessageCircle size={15}/> Checkout on WhatsApp
          </button>
        </div>
      </div>
    </div>
...
function ProductsContent() {
  ...
  // Listen for cart additions to show popup
  const prevCartCount = useMemo(() => cartCount, []); // eslint-disable-line
  useEffect(() => {
    if (cart.length > 0) {
      const last = cart[cart.length - 1];
      if (last) setAddedPopup({ item: last });
    }
      ...
      <div className="container-xl py-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={inStockOnly} onChange={(e) => setInStockOnly(e.target.checked)} className="w-4 h-4 accent-rose-600 rounded"/>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-stone-400 text-sm mb-4">Try adjusting your filters</p>
          ...
          <div className="relative ml-auto w-full max-w-sm bg-white h-full flex flex-col shadow-2xl animate-slide-down">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.selectedColor && (
                      <p className="text-xs text-stone-400 mt-0.5">
                        {item.selectedColor}{item.selectedSize && item.selectedSize !== "Free Size" ? ` · ${item.selectedSize}` : ""}
                      </p>
                    )}
            ...
            {cart.length > 0 && (
              <div className="p-5 border-t border-stone-100 space-y-3">
                ...
                {/* ✅ Working WhatsApp checkout */}
                <button
                  onClick={handleCheckout}
                  className="w-full py-3.5 text-base bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-2xl flex items-center justify-center gap-2.5 transition-colors shadow-md active:scale-95">
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-8 h-8 border-4 border-rose-200 border-t-rose-600 rounded-full animate-spin"/></div>}>
      <ProductsContent/>
    </Suspense>
  );
}
