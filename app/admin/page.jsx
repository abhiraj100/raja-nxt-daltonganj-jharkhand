...
const EMPTY = { name:"", category:"Sarees", price:"", originalPrice:"", images:[], description:"", colors:"", sizes:"", badge:"", inStock:true, featured:false, rating:"", reviews:"" };
const TABS = [
...
export default function AdminPage() {
  ...
  const openEdit = (p) => { const imgs = Array.isArray(p.images) && p.images.length ? p.images : (p.image ? [p.image] : []); setForm({ ...p, images:imgs, colors:p.colors?.join(", ")||"", sizes:p.sizes?.join(", ")||"", badge:p.badge||"", rating:p.rating?.toString()||"", reviews:p.reviews?.toString()||"" }); setEditId(p.id); setFormErrors({}); setTab("add"); };

  const validate = () => {
    const e = {};
    if (!form.name.trim()) e.name = "Required";
    if (!form.price || isNaN(form.price) || Number(form.price) <= 0) e.price = "Valid price required";
    if (!form.images || form.images.length === 0) e.images = "Kam se kam 1 image required hai";
  ...
  const handleSave = () => {
    ...
    const data = { ...form, price:Number(form.price), originalPrice:form.originalPrice?Number(form.originalPrice):null, images:form.images||[], image:(form.images||[])[0]||"", colors:form.colors?form.colors.split(",").map((c)=>c.trim()).filter(Boolean):[], sizes:form.sizes?form.sizes.split(",").map((s)=>s.trim()).filter(Boolean):[], badge:form.badge||null, rating:form.rating?parseFloat(form.rating):0, reviews:form.reviews?parseInt(form.reviews):0 };
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      <tr key={p.id} className={`border-b border-stone-50 hover:bg-stone-50/60 transition-colors last:border-0 ${!p.inStock?"bg-red-50/20":""}`}>
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <img src={p.images?.[0] || p.image} alt={p.name} className="w-10 h-12 object-cover rounded-lg flex-shrink-0 bg-stone-100" onError={(e)=>{e.target.src="https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=80&q=60";}}/>
                            <div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8 max-w-3xl">
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <ImageUploader value={form.images} onChange={(urls) => { setForm((prev) => ({...prev, images:urls})); setFormErrors((e) => ({...e, images:""})); }}/>
                  ...
                  {outOfStock.map((p) => (
                    <div key={p.id} className="bg-white rounded-2xl shadow-sm overflow-hidden border border-red-100">
                      <div className="relative">
                        <img src={p.images?.[0] || p.image} alt={p.name} className="w-full h-40 object-cover opacity-60" onError={(e)=>{e.target.src="https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=400&q=60";}}/>
